...
__global__ void Raykernel(const int w, const int h, unsigned char *dev_bitmap, World* dev_world)
{
	...
	if (x < w && y < h) {
		...
		if (x == 1 && y == 1)printf("%f %f %f %f %f %f %\n", pp*dev_world->camera_ptr->coord, pp*inverseCameraMatrix);
...
__global__ void worldKernel(const int w, const int h, World* dev_world) {
	...
	dev_world->camera_ptr = new Camera_Pinhole(Vec3(0, 100, 300), Vec3(0, 50, 0));
...
__global__ void modifyKernel(int cameraCmd, int cameraRotateX, int cameraRotateY, int cameraZoom, World* dev_world) {
	...
	if (cameraZoom != 0)printf("modifyKernel\n");
	if (cameraZoom != 0)printf("%f %f %f\n", Vec3(0, 300, 300)*dev_world->camera_ptr->coord.inverse());
	if (cameraZoom != 0)printf("%f %f %f\n", Vec3(0, 300, 300)*dev_world->camera_ptr->coord);
